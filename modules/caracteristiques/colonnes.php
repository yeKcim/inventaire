...
global $dbh;
if (!isset($dbh)) {
    throw new \RuntimeException('$dbh non initialisé');
}
...
if (isset($CAT)) {
	if ($CAT=="") {
		...
		if (isset ($tc) && is_array($tc) && array_key_exists($t["base_index"], $tc)) $td.=substr($tc[$t["base_index"]], 0, -2);
		else $td.="-";
	...
	// ICI ? À revoir s’il faut fermer ici le isset($CAT) plutot que juste avant return
